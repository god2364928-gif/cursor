...
const PROGRESS_FILE = '/Users/go/Desktop/hashtag_retry2_progress.json'
const LOG_FILE = '/Users/go/Desktop/hashtag_retry2_log.txt'
...
const needToFetch = emptyHashtags.filter(h => retryProgress[h] === undefined)

log(`전체 해시태그: ${allRows.length}개`)
log(`비어있는 해시태그: ${emptyHashtags.length}개`)
...
log(`예상 소요: ~${Math.ceil(needToFetch.length / CONCURRENCY * 20 / 60)}분`)
log('---')
...
async function processBatch(batch) {
  return Promise.all(batch.map(async (hashtag) => {
    ...
    if (completed % 50 === 0 || completed === needToFetch.length) {
      ...
      log(`${completed}/${needToFetch.length} | 복구: ${recovered} | 존재안함: ${confirmedZero} | 실패: ${stillNull} | ${rate}/s | ~${remaining}분 남음`)
    }
    return result
  }))
}
...
async function main() {
  ...
  log(`\n재시도 완료: ${elapsed}분 소요`)
  log(`복구: ${recovered}, 존재안함: ${confirmedZero}, 실패: ${stillNull}`)

  const resultMap = new Map()
  for (const [h, pc] of Object.entries(retryProgress)) {
    resultMap.set(h, pc)
  }

  const finalRows = allRows.map(r => {
    if (!r.post_count && resultMap.has(r.hashtag)) {
      const pc = resultMap.get(r.hashtag)
      if (pc !== null && pc !== undefined) {
        return { hashtag: r.hashtag, post_count: String(pc) }
      }
    }
    return r
  })
  ...
  const noData = finalRows.filter(r => !r.post_count || r.post_count === 'null')
