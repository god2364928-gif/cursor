...
router.get('/stats', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const contractCustomersResult = await pool.query(contractCustomersQuery, contractCustomersParams)
    ...
    `
    const retargetingAcquiredParams: any[] = []
    ...
    const dbStatusResult = await pool.query(dbStatusQuery, dbStatusParams)
...
router.get('/monthly-sales', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id
    ...
    const personalSales = await pool.query(`
      SELECT 
    ...
    const result = months.map(m => {
      const personal = personalSales.rows.find(row => 
        new Date(row.month).getMonth() + 1 === m.month
      )
      const total = totalSales.rows.find(row => 
        new Date(row.month).getMonth() + 1 === m.month
      )
...
router.get('/performance-stats', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const prevActivityResult = await pool.query(prevTotalActivitiesQuery, prevActivityParams)
