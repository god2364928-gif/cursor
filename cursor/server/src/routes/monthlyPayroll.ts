...
router.put('/update', authMiddleware, adminOnly, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 허용된 필드만 업데이트
    const allowedFields = ['base_salary', 'coconala', 'bonus', 'incentive', 'business_trip', 'rent', 'transport', 'other', 'notes']
    ...
    if (field !== 'notes') {
      ...
      await pool.query(
        `UPDATE monthly_payroll 
         SET total = COALESCE(base_salary, 0) + COALESCE(coconala, 0) + COALESCE(bonus, 0) + COALESCE(incentive, 0) + COALESCE(business_trip, 0) + COALESCE(rent, 0) + COALESCE(transport, 0) + COALESCE(other, 0)
         WHERE id = $1`,
        [id]
      )
    } else {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO monthly_payroll 
       (fiscal_year, month, employee_name, base_salary, coconala, bonus, incentive, business_trip, rent, transport, other, total)
    ...
    for (const employee of employeesResult.rows) {
      ...
      if (existingResult.rows.length > 0) {
        ...
        await pool.query(
          `UPDATE monthly_payroll 
           SET base_salary = $1, 
               total = $1 + COALESCE(coconala, 0) + COALESCE(bonus, 0) + COALESCE(incentive, 0) + COALESCE(business_trip, 0) + COALESCE(rent, 0) + COALESCE(transport, 0) + COALESCE(other, 0),
               updated_at = CURRENT_TIMESTAMP
        ...
        await pool.query(
          `INSERT INTO monthly_payroll 
           (fiscal_year, month, employee_name, base_salary, coconala, bonus, incentive, business_trip, rent, transport, other, total)
           VALUES ($1, $2, $3, $4, 0, 0, 0, 0, 0, 0, 0, $4)`,
...
// 파일 업로드 (DB 저장) — 다중 파일 지원
router.post('/file/upload', authMiddleware, adminOnly, upload.array('files', 20), async (req: AuthRequest, res: Response) => {
  try {
    const { fiscalYear, month } = req.body
    ...
    try {
      ...
      // 기존 파일은 유지하고 새 파일을 추가 (월별 여러 파일 허용)
...
// 파일 다운로드 (id 기준)
// 주의: '/file/:fiscalYear/:month' 목록 라우트보다 먼저 등록해야 '/file/download/:id' 가 가로채지지 않음
router.get('/file/download/:id', authMiddleware, adminOnly, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params

    const result = await pool.query(
      `SELECT file_name, file_data, mime_type FROM monthly_payroll_files WHERE id = $1`,
      [id]
    )
...
// 파일 삭제 (id 기준)
router.delete('/file/:id', authMiddleware, adminOnly, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params

    const result = await pool.query(
      `DELETE FROM monthly_payroll_files WHERE id = $1 RETURNING id`,
      [id]
    )
