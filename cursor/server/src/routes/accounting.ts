  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO accounting_transactions 
       ...
       RETURNING *`,
      [transactionDate, transactionType, category, paymentMethod, itemName, amount, employeeId || null, accountId || null, memo || null, attachmentUrl || null]
    )
    ...
    if (category === '매출' && transactionType === '입금') {
      ...
      await pool.query(
        ...
        [fiscalYear, month, paymentMethod, amount]
      )
    }
  ...
  try {
    ...
    const salesResult = await pool.query(
      `SELECT COALESCE(SUM(amount), 0) as total_sales
       FROM accounting_transactions
       WHERE TO_CHAR(transaction_date, 'YYYY-MM') = $1 AND category = '매출'`,
