...
function postTranslationFix(data: any): void {
  ...
  const fixes: [string, string][] = [
    ...
    ['現在ハッシュタグを維持してください', '現在のハッシュタグを維持してください'],
    ['個の目標をアップロード！あと', '個を目標にアップロード！あと'],
    ['コンテンツが自然に消費されます', 'コンテンツも自然に閲覧されます'],
    ['コンテンツプールの確保', 'コンテンツをストック'],
    ['休眠状態ではありませんが', '更新が止まっている状態ではありませんが'],
    ['アップロードルーチンの確立', '投稿スケジュールの確立'],
    ['核心インサイト', '主要インサイト'],
    ['曖昧', '一部適合'],
  ]
...
router.get('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (req.user?.id) {
      logFeatureUsage(req.user.id, '계정최적화조회2.0', { query: id })
    }
