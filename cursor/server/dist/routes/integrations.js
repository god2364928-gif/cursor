    ...
    try {
        ...
        while (true) {
            ...
            for (const r of data) {
                ...
                const manualExisting = await db_1.pool.query(`SELECT id, company_name, account_id FROM sales_tracking
           WHERE manager_name = $1 AND date = $2 AND phone = $3
             AND (external_call_id IS NULL OR TRIM(external_call_id) = '')
           ORDER BY created_at ASC LIMIT 1`, [managerName, dateStr, phoneNum]);
                const occurredAt = (() => {
                    if (r.created_at) {
            ...
            WHERE external_call_id = $6`, [dateStr, occurredAt, managerName, companyName, phoneNum, externalId]);
              ...
              external_source = 'cpi',
              updated_at = NOW()
            WHERE id = $7`, [dateStr, occurredAt, managerName, newCompany, phoneNum, externalId, manual.id]);
                    continue;
                }
