...
router.get('/stats', auth_1.authMiddleware, async (req, res) => {
    try {
        // 전체 데이터 수
        const totalResult = await db_1.pool.query('SELECT COUNT(*) FROM inquiry_leads');
        const total = parseInt(totalResult.rows[0].count);
        // 미배정 데이터 수
        const unassignedResult = await db_1.pool.query("SELECT COUNT(*) FROM inquiry_leads WHERE assignee_id IS NULL AND status = 'PENDING'");
        const unassigned = parseInt(unassignedResult.rows[0].count);
        // 금주 완료 수 (이번 주 월요일부터) - 완료/홈페이지없음/문의하기없음/기타 모두 포함
        const weekStartResult = await db_1.pool.query(`
    ...
    `);
        ...
        // 전체 진행률 (완료 / 배정된 전체)
        const assignedResult = await db_1.pool.query("SELECT COUNT(*) FROM inquiry_leads WHERE assignee_id IS NOT NULL");
        const assigned = parseInt(assignedResult.rows[0].count);
        const completedResult = await db_1.pool.query("SELECT COUNT(*) FROM inquiry_leads WHERE status IN ('COMPLETED', 'NO_SITE', 'NO_FORM', 'ETC')");
        const completed = parseInt(completedResult.rows[0].count);
        const progressRate = assigned > 0 ? Math.round((completed / assigned) * 100) : 0;
        // 담당자별 통계
        const assigneeStatsResult = await db_1.pool.query(`
      SELECT 
        u.id,
        u.name,
...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        const { page = '1', limit = '50', assigneeId, status, prefecture, search } = req.query;
 ...
 */
router.post('/bulk-assign', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        // 담당자 존재 확인
        const userResult = await db_1.pool.query('SELECT id, name FROM users WHERE id = $1', [assigneeId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({ message: 'User not found' });
        }
        // 미배정 데이터 중 count개 랜덤으로 가져오기
        const leadsResult = await db_1.pool.query(`
...
router.put('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        // 리드 존재 확인
        const leadResult = await db_1.pool.query('SELECT id FROM inquiry_leads WHERE id = $1', [id]);
 ...
 */
router.get('/assignees', auth_1.authMiddleware, async (req, res) => {
    try {
 ...
 */
router.get('/prefectures', auth_1.authMiddleware, async (req, res) => {
    try {
        const result = await db_1.pool.query(`
...
//# sourceMappingURL=inquiryLeads.js.map