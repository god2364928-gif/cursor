...
const hotpepperClient_1 = require("../integrations/hotpepperClient");
const hotpepperCrawler_1 = require("../services/hotpepperCrawler");
...
router.post('/search', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (area && hotpepperClient_1.AREA_CODES[area]) {
            searchParams.large_area = hotpepperClient_1.AREA_CODES[area];
        }
        // API 호출
        const result = await (0, hotpepperClient_1.searchRestaurants)(searchParams);
        ...
        for (const shop of shops) {
            try {
                const formattedData = (0, hotpepperClient_1.formatRestaurantForDB)(shop, keyword, area);
                // 중복 체크 (hotpepper_id 기준)
                const existingCheck = await db_1.pool.query('SELECT id FROM hotpepper_restaurants WHERE hotpepper_id = $1 AND is_deleted = false', [formattedData.hotpepper_id]);
                if (existingCheck.rows.length > 0) {
                    // 이미 존재하는 경우 업데이트
                    await db_1.pool.query(`UPDATE hotpepper_restaurants 
             ...
             WHERE hotpepper_id = $9`, [
                    ...
                    // 새로 저장
                    await db_1.pool.query(`INSERT INTO hotpepper_restaurants 
 ...
 */
router.get('/restaurants', auth_1.authMiddleware, async (req, res) => {
    try {
 ...
 */
router.get('/restaurants/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await db_1.pool.query(`SELECT * FROM hotpepper_restaurants WHERE id = $1 AND is_deleted = false`, [id]);
 ...
 */
router.patch('/restaurants/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { notes } = req.body;
        await db_1.pool.query(`UPDATE hotpepper_restaurants SET notes = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`, [notes || null, id]);
 ...
 */
router.delete('/restaurants/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.pool.query(`UPDATE hotpepper_restaurants SET is_deleted = true, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [id]);
        res.json({ success: true });
    }
    catch (error) {
 ...
 */
router.post('/restaurants/bulk-delete', auth_1.authMiddleware, async (req, res) => {
    try {
        const { ids } = req.body;
        if (!Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({ message: 'Invalid IDs' });
        }
        await db_1.pool.query(`UPDATE hotpepper_restaurants SET is_deleted = true, updated_at = CURRENT_TIMESTAMP 
 ...
 */
router.post('/crawl-details', auth_1.authMiddleware, async (req, res) => {
    try {
    ...
    `);
        ...
        // 백그라운드에서 크롤링 실행
        (0, hotpepperCrawler_1.crawlHotpepperDetails)(batchSize)
            .then(result => {
 ...
 */
router.get('/crawl-status', auth_1.authMiddleware, async (req, res) => {
    try {
        // 전화번호가 있는 레코드 수
        const withTelResult = await db_1.pool.query(`
      SELECT COUNT(*) as count
      FROM hotpepper_restaurants
    ...
    `);
        // 전화번호가 없는 레코드 수
        const withoutTelResult = await db_1.pool.query(`
      SELECT COUNT(*) as count
      FROM hotpepper_restaurants
    ...
    `);
        // 공식 홈페이지가 있는 레코드 수
        const withHomepageResult = await db_1.pool.query(`
      SELECT COUNT(*) as count
      FROM hotpepper_restaurants
...
//# sourceMappingURL=hotpepper.js.map