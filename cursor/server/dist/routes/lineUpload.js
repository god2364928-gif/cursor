...
router.post('/', auth_1.authMiddleware, upload.single('file'), async (req, res) => {
    try {
        ...
        console.log('===============================\n');
        // LINE 대화 파싱
        const parsed = (0, lineParser_1.parseLineChat)(text);
        // DEBUG: 파싱 결과 로그
 ...
 * 영업 이력: 새 고객으로 등록
 */
async function handleSalesHistory(parsed, req) {
    ...
    const memo = (0, lineParser_1.formatChatForHistory)(parsed);
 ...
 */
async function handleRetargeting(parsed, req) {
    const companyName = parsed.extractedCompanyName;
    if (!companyName) {
        throw new Error('상호명을 찾을 수 없습니다. LINE 대화에 회사명이 포함되어 있는지 확인해주세요.');
    }
    // 상호명으로 고객 찾기
    const customerResult = await db_1.pool.query('SELECT id, company_name, customer_name FROM retargeting_customers WHERE company_name ILIKE $1', [`%${companyName}%`]);
    if (customerResult.rows.length === 0) {
        throw new Error(`"${companyName}"과(와) 일치하는 리타겟팅 고객을 찾을 수 없습니다.`);
    }
    const customer = customerResult.rows[0];
    const historyContent = (0, lineParser_1.formatChatForHistory)(parsed);
    // 히스토리 추가
    await db_1.pool.query(`INSERT INTO retargeting_history (
 ...
 */
async function handleCustomerManagement(parsed, req) {
    const companyName = parsed.extractedCompanyName;
    if (!companyName) {
        throw new Error('상호명을 찾을 수 없습니다. LINE 대화에 회사명이 포함되어 있는지 확인해주세요.');
    }
    // 상호명으로 고객 찾기
    const customerResult = await db_1.pool.query('SELECT id, company_name, customer_name FROM customers WHERE company_name ILIKE $1', [`%${companyName}%`]);
    if (customerResult.rows.length === 0) {
...
//# sourceMappingURL=lineUpload.js.map