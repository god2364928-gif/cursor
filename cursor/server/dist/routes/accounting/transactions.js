...
router.post('/transactions', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { transactionDate, transactionTime, transactionType, category, paymentMethod, itemName, amount, employeeId, accountId, assignedUserId, memo, attachmentUrl, bankName, } = req.body;
       ...
       RETURNING *`, [
        ...
        if (transactionType === '입금' && salesCategories.includes(category)) {
            ...
            const dateStr = (0, dateHelper_1.toJSTDateString)(new Date(transactionDate));
...
router.post('/transactions/bulk', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.post('/transactions/bulk-update', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.post('/transactions/bulk-delete', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.put('/transactions/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // 기존 데이터 조회
        const existingResult = await db_1.pool.query(`SELECT * FROM accounting_transactions WHERE id = $1`, [id]);
        if (existingResult.rows.length === 0) {
...
router.delete('/transactions/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
...
// ========== CSV 업로드 (은행 거래내역) ==========
router.post('/transactions/csv-upload', auth_1.authMiddleware, adminOnly_1.adminOnly, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
       ...
       ORDER BY priority DESC, keyword ASC`);
        ...
        for (const cols of dataLines) {
           ...
           RETURNING *`, [transactionDate, transactionTime, transactionType, category, paymentMethod, itemName, amount, assignedUserId, memoClean, 'PayPay']);
...
router.delete('/transactions/all', auth_1.authMiddleware, adminOnly_1.adminOnly, async (_req, res) => {
    try {
...
// ========== SMBC 은행 붙여넣기 업로드 ==========
router.post('/transactions/smbc-paste', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
       ...
       ORDER BY priority DESC, keyword ASC`);
        ...
        while (i < lines.length) {
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
           RETURNING *`, [transactionDate, transactionType, category, paymentMethod, itemName, amount, assignedUserId, null, 'SMBC']);
                imported.push(result.rows[0]);
                // 다음 "入金" 또는 "出金" 찾기
                i = idx;
