...
router.get('/recurring-expenses', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.post('/recurring-expenses', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.put('/recurring-expenses/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/recurring-expenses/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
...
// ========== 계좌 (Capital) ==========
router.get('/capital', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.post('/capital', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.put('/capital/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        const { accountName, accountType, initialBalance } = req.body;
...
router.delete('/capital/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/auto-match-rules', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.post('/auto-match-rules', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
router.put('/auto-match-rules/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        const { keyword, category, assignedUserId, paymentMethod, priority, isActive } = req.body;
        if (!keyword || keyword.trim() === '') {
...
router.delete('/auto-match-rules/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/capital-balance', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const limit = req.query.limit ? Number(req.query.limit) : 12;
        const offset = req.query.offset ? Number(req.query.offset) : 0;
        const result = await db_1.pool.query(`SELECT id, balance_date, amount, note, created_at, updated_at
...
router.post('/capital-balance', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`INSERT INTO capital_balance (balance_date, amount, note)
       VALUES ($1, $2, $3)
       RETURNING *`, [balanceDate, amount, note || null]);
...
router.put('/capital-balance/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        const { balanceDate, amount, note } = req.body;
        // Validate date is the 1st of the month
        ...
        const result = await db_1.pool.query(`UPDATE capital_balance
       SET balance_date = $1,
           amount = $2,
           note = $3,
           updated_at = NOW()
       WHERE id = $4
       RETURNING *`, [balanceDate, amount, note || null, id]);
        if (result.rows.length === 0) {
...
router.delete('/capital-balance/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.pool.query(`DELETE FROM capital_balance WHERE id = $1`, [id]);
        res.json({ success: true });
    }
    catch (error) {
...
// ========== 보증금 (Deposits) ==========
// GET /deposits
router.get('/deposits', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
...
// POST /deposits
router.post('/deposits', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { itemName, amount, note } = req.body;
        if (!itemName || itemName.trim() === '') {
            return res.status(400).json({ error: '항목명은 필수입니다' });
        }
        const result = await db_1.pool.query(`INSERT INTO deposits (item_name, amount, note)
       VALUES ($1, $2, $3)
       RETURNING *`, [itemName.trim(), amount, note || null]);
        res.json(result.rows[0]);
    }
    catch (error) {
...
// PUT /deposits/:id
router.put('/deposits/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        const { itemName, amount, note } = req.body;
        if (!itemName || itemName.trim() === '') {
        ...
        const result = await db_1.pool.query(`UPDATE deposits
       SET item_name = $1,
           amount = $2,
           note = $3,
           updated_at = NOW()
       WHERE id = $4
       RETURNING *`, [itemName.trim(), amount, note || null, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: '데이터를 찾을 수 없습니다' });
        }
        res.json(result.rows[0]);
...
// DELETE /deposits/:id
router.delete('/deposits/:id', auth_1.authMiddleware, adminOnly_1.adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.pool.query(`DELETE FROM deposits WHERE id = $1`, [id]);
        res.json({ success: true });
    }
    catch (error) {
