...
router.get('/performance-stats', auth_1.authMiddleware, async (req, res) => {
    ...
    `;
        ...
        // 병렬 쿼리 실행
        const [formResult, contactMethodResult, retargetingResult, existingResult, salesResult, prevSalesResult] = await Promise.all([
    ...
    `;
        ...
        const stageResult = await db_1.pool.query(retargetingStageQuery, stageParams);
        // === 5. 리타 계약률 계산 ===
      ...
      ${manager && manager !== 'all' ? `AND u.name = $3` : ''}
    `;
        const retargetingContractResult = await db_1.pool.query(retargetingContractQuery, manager && manager !== 'all' ? [validatedStartDate, validatedEndDate, manager] : [validatedStartDate, validatedEndDate]);
      ...
      ${manager && manager !== 'all' ? `AND u.name = $3` : ''}
    `;
        const prevMonthSalesResult = await db_1.pool.query(prevMonthSalesQuery, manager && manager !== 'all'
    ...
    `;
        ...
        const alertResult = await db_1.pool.query(retargetingAlertQuery, alertParams);
        // 리타겟팅 단계별 고객 수
