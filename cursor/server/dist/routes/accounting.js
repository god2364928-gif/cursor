...
router.get('/dashboard', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
       ...
       ORDER BY month, category`, [twelveMonthsAgoStr, nowStr]);
        // 월별 카테고리별 매출 (최근 12개월)
        const monthlySalesByCategoryResult = await db_1.pool.query(`SELECT 
        TO_CHAR(transaction_date, 'YYYY-MM') as month,
        category,
        COALESCE(SUM(amount), 0) as total
       FROM accounting_transactions
...
router.post('/transactions/bulk', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
...
router.post('/transactions/bulk-update', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        ...
        for (const transactionId of transactionIds) {
            try {
                // 기존 데이터 조회
                const existingResult = await db_1.pool.query(`SELECT * FROM accounting_transactions WHERE id = $1`, [transactionId]);
                ...
                // 업데이트 실행
                await db_1.pool.query(`UPDATE accounting_transactions 
           SET category = $1, 
               assigned_user_id = $2
           WHERE id = $3`, [newCategory, newAssignedUserId, transactionId]);
                updatedCount++;
            }
            catch (error) {
...
router.post('/transactions/bulk-delete', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        ...
        for (const transactionId of transactionIds) {
            try {
                await db_1.pool.query(`DELETE FROM accounting_transactions WHERE id = $1`, [transactionId]);
                deletedCount++;
            }
            catch (error) {
...
// ========== CSV 업로드 (은행 거래내역) ==========
router.post('/transactions/upload-csv', auth_1.authMiddleware, adminOnly, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
...
router.get('/capital-balance', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
...
router.post('/capital-balance', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
...
router.put('/capital-balance/:id', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        const { balanceDate, amount, note } = req.body;
        // Validate date is the 1st of the month
...
router.delete('/capital-balance/:id', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
...
// ========== 보증금 (Deposits) ==========
// GET /deposits
router.get('/deposits', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
...
// POST /deposits
router.post('/deposits', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
...
// PUT /deposits/:id
router.put('/deposits/:id', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
        const { itemName, amount, note } = req.body;
        if (!itemName || itemName.trim() === '') {
...
// DELETE /deposits/:id
router.delete('/deposits/:id', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/employees/:userId/files', auth_1.authMiddleware, adminOnly, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
...
router.get('/employees/:userId/files', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        const { userId } = req.params;
...
router.get('/employees/:userId/files/:fileId', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
...
router.delete('/employees/:userId/files/:fileId', auth_1.authMiddleware, adminOnly, async (req, res) => {
    try {
        const { userId, fileId } = req.params;
