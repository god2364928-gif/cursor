...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const salesTrackingQuery = partialKeyword
          ...
          CASE WHEN customer_name = $1 OR account_id = $1 OR phone = $1 OR contact_person = $1 OR regexp_replace(phone, '[^0-9]', '', 'g') = regexp_replace($1, '[^0-9]', '', 'g') THEN 1
               WHEN customer_name ILIKE $2 OR account_id ILIKE $2 OR phone ILIKE $2 OR contact_person ILIKE $2 OR regexp_replace(phone, '[^0-9]', '', 'g') LIKE regexp_replace($2, '[^0-9]', '', 'g') || '' THEN 2
               WHEN customer_name ILIKE $3 OR account_id ILIKE $3 OR phone ILIKE $3 OR contact_person ILIKE $3 THEN 3 END as match_priority
        FROM sales_tracking
        WHERE (customer_name = $1 OR account_id = $1 OR phone = $1 OR contact_person = $1 OR regexp_replace(phone, '[^0-9]', '', 'g') = regexp_replace($1, '[^0-9]', '', 'g'))
           OR (customer_name ILIKE $2 OR account_id ILIKE $2 OR phone ILIKE $2 OR contact_person ILIKE $2 OR regexp_replace(phone, '[^0-9]', '', 'g') LIKE regexp_replace($2, '[^0-9]', '', 'g') || '')
        ...
        ORDER BY match_priority, date DESC LIMIT 10`
            : `SELECT 'salesTracking' as page, manager_name, COALESCE(customer_name, account_id, '(no name)') as display_name, id,
          CASE WHEN customer_name = $1 OR account_id = $1 OR phone = $1 OR contact_person = $1 OR regexp_replace(phone, '[^0-9]', '', 'g') = regexp_replace($1, '[^0-9]', '', 'g') THEN 1
               WHEN customer_name ILIKE $2 OR account_id ILIKE $2 OR phone ILIKE $2 OR contact_person ILIKE $2 OR regexp_replace(phone, '[^0-9]', '', 'g') LIKE regexp_replace($2, '[^0-9]', '', 'g') || '' THEN 2 END as match_priority
        FROM sales_tracking
        WHERE (customer_name = $1 OR account_id = $1 OR phone = $1 OR contact_person = $1 OR regexp_replace(phone, '[^0-9]', '', 'g') = regexp_replace($1, '[^0-9]', '', 'g'))
           OR (customer_name ILIKE $2 OR account_id ILIKE $2 OR phone ILIKE $2 OR contact_person ILIKE $2 OR regexp_replace(phone, '[^0-9]', '', 'g') LIKE regexp_replace($2, '[^0-9]', '', 'g') || '')
        ORDER BY match_priority, date DESC LIMIT 10`;
