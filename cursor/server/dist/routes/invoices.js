...
router.post('/create', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        partner_name, partner_title, invoice_title, invoice_date, due_date, tax_entry_method, line_items, payment_bank_info, payment_method, // 추가: 결제 방식 (bank/paypay/paypal)
        memo, // 추가: 비고
         } = req.body;
        // 입력 유효성 검사
        ...
        if (payment_method === 'paypal') {
            (0, slackClient_1.sendPaypalInvoiceNotification)({
...
router.post('/:id/cancel', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        // 1. 청구서 조회
        const result = await db_1.pool.query(`SELECT i.*, u.name as user_name 
       FROM invoices i 
       LEFT JOIN users u ON i.user_id = u.id 
       WHERE i.id = $1`, [id]);
        ...
        await db_1.pool.query(`UPDATE invoices 
       SET is_cancelled = true, 
           cancelled_at = $1, 
           cancelled_by_user_id = $2 
       WHERE id = $3`, [cancelledAt, userId, id]);
        console.log(`✅ Invoice cancelled: ${id} by user ${userId}`);
        // 6. 사용자 이름 조회
        const userResult = await db_1.pool.query('SELECT name FROM users WHERE id = $1', [userId]);
        const userName = userResult.rows[0]?.name || '알 수 없음';
        // 7. 슬랙 알림 전송 (비동기, 실패해도 취소에는 영향 없음)
        (0, slackClient_1.sendInvoiceCancelNotification)({
...
router.get('/:id/pdf', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('SELECT freee_invoice_id, company_id, due_date, memo, payment_bank_info, tax_entry_method FROM invoices WHERE id = $1', [id]);
        if (result.rows.length === 0) {
        ...
        const pdfBuffer = await (0, freeeClient_1.downloadInvoicePdf)(Number(company_id), Number(freee_invoice_id), due_date, memo, payment_bank_info, tax_entry_method);
