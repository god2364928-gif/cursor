 ...
 */
router.get('/genres', auth_1.authMiddleware, async (req, res) => {
    try {
        const result = await db_1.pool.query(`
...
router.get('/:id', auth_1.authMiddleware, async (req, res) => {
    ...
    `, [id]);
        ...
        // 영업 이력 조회
        const activitiesResult = await db_1.pool.query(`
      SELECT 
        sa.id,
        sa.user_name,
        sa.contact_method,
...
router.patch('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        params.push(id);
        await db_1.pool.query(`UPDATE restaurants SET ${updates.join(', ')} WHERE id = $${paramIndex}`, params);
 ...
 */
router.post('/:id/unusable', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.delete('/:id/unusable', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.pool.query(`
      UPDATE restaurants 
 ...
 */
router.post('/:id/sales-activity', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // 가게 정보 조회
        const restaurantResult = await db_1.pool.query(`
    ...
    `, [id]);
        ...
        const activityResult = await db_1.pool.query(`
 ...
 */
router.get('/:id/sales-activities', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await db_1.pool.query(`
      SELECT 
